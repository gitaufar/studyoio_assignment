...
export const Table: React.FC<TableProps> = ({ data, columns, onEdit, onDelete, emptyMessage }) => {
  if (data.length === 0) {
    return (
      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          ...
          <tbody className="divide-y divide-gray-200 dark:divide-dark-border">
            {data.map((row, index) => (
              <tr key={row.id || index} className="hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                {columns.map((column) => (
